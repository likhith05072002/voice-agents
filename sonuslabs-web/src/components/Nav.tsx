    ...
    <div style={{ position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(10px)",
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 28px", height: 66,
        ...
        <div onClick={() => nav("/")} style={{ display: "flex", alignItems: "center", gap: 10,
          ...
          <span style={{ fontFamily: serif, fontSize: 23, letterSpacing: ".2px" }}>SonusLabs</span>
        </div>
        <span onClick={() => nav("/")} style={link}>Product</span>
        <span onClick={() => nav("/#pricing")} style={link}>Pricing</span>
        <span onClick={() => nav("/console")} style={link}>Console</span>
        <button onClick={() => nav("/create")} style={{ fontSize: 14, fontWeight: 600, color: "#fff",
          background: C.ink, border: "none", borderRadius: 11, padding: "10px 17px", cursor: "pointer",
          whiteSpace: "nowrap" }}>Create your receptionist</button>
      </div>
    </div>
  );
}
